...
SELECT app_name AS most_reviewed_apps, reviews FROM analytics LIMIT 5;
...
SELECT app_name, last_updated FROM analytics WHERE last_updated IN (SELECT MIN(last_updated) FROM analytics);

-- Find the most expensive app (the query is very similar to #11).--
SELECT app_name, price FROM analytics WHERE price IN (SELECT MAX(price) FROM analytics);

-- Count all the reviews in the Google Play Store.--
SELECT SUM(reviews) AS all_google-play_app_reviews FROM analytics;

-- Find all the categories that have more than 300 apps in them.--
SELECT category, COUNT(app_name) FROM analytics GROUP BY category HAVING COUNT(*) > 300;
